    ...
    public static class WebHelper
    {
            ...
            string proxy = null, string userAgent = null, int requestTimeOut = 0, bool needZip = false)
        {
            ...
            if (needZip)
            {
                ...
                WST.Core.Util.ZipHelper.ZipFiles(zipfilepath, filePath);
        ...
        public static void UploadFileServer(UploadSuccessHandle method)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(uploadlast) && uploadlast == "1")
                {
                    ...
                    {
                        //WST.Core.Util.LogHelper.WriteCustom(zipName + " " + fileTmp + " " + serverFileName, null, null);
                        // 解压
        ...
        /// <param name="proxy">WebRequest要用到的代理</param>
        /// <returns></returns>
        public static string GetPage(string url, string para, string httpMethod)
        {
            return GetPage(url, "", para, httpMethod, Encoding.UTF8);
        }
        /// <summary>
        /// 抓取页面
        /// </summary>
