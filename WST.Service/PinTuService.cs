...
namespace WST.Service
{
    ...
    /// 拼图
    /// </summary>
    public class PinTuService : BaseService<PinTu>, IPinTuService
    {
        public PinTuService()
        {
        ...
        /// <returns></returns>
        public PageList<PinTu> GetPageList(int pageIndex, int pageSize,string userId, string name,string userName)
        {
            using (DbRepository db = new DbRepository())
            {
                var query = db.PinTu.Where(x => !x.IsDelete);
        ...
        public List<SelectItem> GetSelectList()
        {
            using (DbRepository db = new DbRepository())
            {
                return db.PinTu.Where(x => !x.IsDelete).OrderByDescending(x=>x.CreatedTime).Select(x => new SelectItem()
                {
                    Text = x.Name,
                    Value = x.ID,
                }).ToList(); ;
