...
namespace WST.Service
{
    ...
    public class ActivityCategoryService : BaseService<ActivityCategory>, IActivityCategoryService
    {
        ...
        /// <returns></returns>
        public PageList<ActivityCategory> GetPageList(int pageIndex, int pageSize, string name)
        {
            using (DbRepository db = new DbRepository())
            {
                var query = db.ActivityCategory.Where(x => !x.IsDelete);
        ...
        public List<SelectItem> GetSelectList()
        {
            using (DbRepository db = new DbRepository())
            {
                return db.ActivityCategory.Where(x => !x.IsDelete).OrderByDescending(x=>x.Sort).Select(x => new SelectItem()
                {
                    Text = x.Name,
                    Value = x.ID,
                }).ToList(); ;
