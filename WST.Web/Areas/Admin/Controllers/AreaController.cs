...
namespace WST.Web.Areas.Admin.Controllers
{
    public class AreaController : BaseAdminController
    {
        ...
        {
            var result = IDataDictionaryService.Delete(ids);
            if (result > 0)
            {
                CacheHelper.Clear();
            }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetAllSelectItem(string value = "")
        {
            if (value.Length != 6)
                return DataErorrJResult();
            var provinceList = IDataDictionaryService.GetSelectList(GroupCode.Area, "");
            var cityList = IDataDictionaryService.GetSelectList(GroupCode.Area, value.Substring(0, 2) + "0000");
            var countyList = IDataDictionaryService.GetSelectList(GroupCode.Area, value.Substring(0, 4) + "00");
            return JResult(
                new
                {
