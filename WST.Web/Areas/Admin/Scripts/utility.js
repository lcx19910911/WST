...
function ValidTime(start, end,limitTime)
{
    var startTime = new Date("1991-09-11 "+start+":00");
    var endTime = new Date("1991-09-11 " + end + ":00");
    if (endTime - startTime > limitTime) {
        return "开始时间" + start + "和结束时间" + end + "必须在" + limitTime / 60000 + "分钟内";
    }
    else {
        return "";
    }
    //if ((endTime - startTime) / 150000 != 0)
    //{
    //    return "开始时间" + start + "和结束时间" + end + "的时间差必须为15分钟的整数倍";
    //}
...
function ShowUserHead(item) {
    return "<img src='" + item.HeadImgUrl + "' style='width:30px;height:40px;'/>"
}

function ShowPic(item) {
    return "<img src='" + item.Picture + "' style='width:30px;height:40px;'/>"
}
